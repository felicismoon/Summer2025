...
double protonMass = PhysicsConstants.massProton();
double theZ = 0;
...
// different ways of getting these limits
	...
	// Particle gamma0 = physEvent.getParticleByPid(22,0);
	 ...
	 double theZ = (totalE + totalPz) / (protonMass + 2*nu);


 		 if(W>2.0&&Q2>1.0&&y<0.85&&theZ<0.13){
			filter3++;
 		...
 		if(W>2.0&&Q2>1.0&&y<0.85&&theZ>=.13&&theZ<.195){
			filter3++;
			vL_omega.copy(vL_pi0_1);
			vL_omega.add(piPlus.vector());
			vL_omega.add(piMinus.vector());
		...
		if(W>2.0&&Q2>1.0&&y<0.85&&theZ>=.195){
			filter3++;
			vL_omega.copy(vL_pi0_1);
			vL_omega.add(piPlus.vector());
			vL_omega.add(piMinus.vector());
...
//////////////////////original
/*
F1D f4 = new F1D("f4","[amp]*gaus(x,[mean],[sigma])", .71, .82);



//set the initial values for the fit
//f4.setParameter(0, 17); // value for [amp]
f4.setParameter(0, 15); // value for [amp]
f4.setParameter(1, 0.78);   // value for [mean]
f4.setParameter(2, 0.02);  // value for [sigma]
*/
////////////////////////////////////////////////testing
...
// gives a broad cu58.627rved orange line
//F1D f6 = new F1D("f6","[amp]*gaus(x,[mean],[sigma])+[B0]+[B1]*x+[B2]*x*x", lowerlimit2, upperlimit2);
//F1D f6 = new F1D("f6","[amp]*gaus(x,[mean],[sigma])+[B0]+[B1]*x+[B2]*x*x", .5, .9);

//F1D f6 = new F1D("f6","[amp]*gaus(x,[mean],[sigma])+[B0]+[B1]*x+[B2]*x*x", .7, .85);

F1D f6 = new F1D("f6","[amp]*gaus(x,[mean],[sigma])+[B0]+[B1]*x+[B2]*x*x", .65, .84);
...
//F1D f8 = new F1D("f8","[amp]*gaus(x,[mean],[sigma])", .71, .85);

F1D f8 = new F1D("f8","[amp]*gaus(x,[mean],[sigma])", .7, .82);

//set the initial values for the fit
f8.setParameter(0, 17); // value for [amp]
//f8.setParameter(0, 100); // value for [amp]
...
//F1D f10 = new F1D("f10","[amp]*gaus(x,[mean],[sigma])+[B0]+[B1]*x+[B2]*x*x", .7, .9);

F1D f10 = new F1D("f10","[amp]*gaus(x,[mean],[sigma])+[B0]+[B1]*x+[B2]*x*x", .71, .84);
...
//F1D f13 = new F1D("f13","[amp]*gaus(x,[mean],[sigma])+[B0]+[B1]*x+[B2]*x*x", .7, .85);
