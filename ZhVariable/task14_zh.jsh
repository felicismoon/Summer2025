// for figuring out three equal bins of nu. you change the parameters of nu for each target (LC, SC, LPb, SPb), until the .getIntegral() for each bin/histogram is about the same
...
// change values of zh here until each histogram (hz1, hz2, hz3) all have about the same amount of data in them (.getIntegral() is about the same)
