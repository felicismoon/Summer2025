// gets omega invariant mass for all data with pi0 cut, theta cut, and DIS cut
...
//reader.getEvent(event,0); // Reads the first event and resets to the begining <-- use this if you're using normal hipo reader with just one file
...
// getting the angles: values from task4_3. I used two sigma here
...
// gets theta cut between the two gammas and the electron
...
// theta used according to visual inspection -- can test with thetatest program 		
...
// here: a fit function with both the hist and the linear fit - gives a broad curved orange line
