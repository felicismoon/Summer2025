// finds three sigma values for Vz for Lead. Vz lets you select between the liquid and the solid target
...
f4.setParameter(0, 60000.0); // value for [amp]
...
f7.setParameter(0, 36000.0); // value for [amp]
f7.setParameter(1, -1.0);   // value for [mean]
f7.setParameter(2, .8);  // value for [sigma]
...
F1D f5 = new F1D("f5","[B0]+[B1]*x+[B2]*x*x", -12, -3);

//F1D f5 = new F1D("f5","[B0]+[B1]*x+[B2]*x*x", lowerlimit2, upperlimit2);


f5.setParameter(0, 2000); // value for [B0]
...
F1D f8 = new F1D("f8","[B0]+[B1]*x+[B2]*x*x", -3, 3);
