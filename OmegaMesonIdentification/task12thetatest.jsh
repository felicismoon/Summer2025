// this program is for checking the value of theta you're cutting on. one of the graphs creates the omega peak using the values of theta you chose, while the other one shows the data that got cut. the goal is to make sure that an omega peak isn't showing up in what gets cut or the theta cut is too high. However, in practice, I didn't actually see an omega cut showing up in the histogram of the cut data. Instead, while raising the theta cut made the omega peak more pronounced and more clear, the actual height of the peak would eventually decrease because there was more data being cut from it. My decision was ultimately to use the theta cut with the highest height of the omega peak
...
// following pi0 values from 4_3. using two sigma here.
	  ...
	  // if(theta0>=0.2&&theta1>=0.2){
	   // if(theta0>=0.3&&theta1>=0.3){
		//if(theta0>=0.4&&theta1>=0.4){
	     //if(theta0>=0.01&&theta1>=0.01){
	    //if(theta0>=0.5&&theta1>=0.5){
	    if(theta0>=0.35&&theta1>=0.35){
  	    //if(theta0>=0.325&&theta1>=0.325){
...
// this is the data that didn't make it through the theta cut
...
// creates histo of invariant mass from normal data
...
// creates histo of invariant mass from leftover data

TCanvas ec6 = new TCanvas("ec2",800,400);


hpi1_2.setTitleX("M(#gamma#gamma #pi+ #pi-) [GeV]");
hpi1_2.setTitleY("Counts");
hpi1_2.setTitle("Leftover");
