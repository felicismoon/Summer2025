...
// different ways of getting these limits
	...
	// Particle gamma0 = physEvent.getParticleByPid(22,0);
	 ...
	 double pT2 = totalPx2 + totalPy2;
 		...
 		if(W>2.0&&Q2>1.0&&y<0.85&&pT2>=.0525&&pT2<.142){
			filter3++;
			vL_omega.copy(vL_pi0_1);
			vL_omega.add(piPlus.vector());
			vL_omega.add(piMinus.vector());
		...
		if(W>2.0&&Q2>1.0&&y<0.85&&pT2>=.142){
			filter3++;
			vL_omega.copy(vL_pi0_1);
			vL_omega.add(piPlus.vector());
			vL_omega.add(piMinus.vector());
...
F1D f10 = new F1D("f10","[amp]*gaus(x,[mean],[sigma])+[B0]+[B1]*x+[B2]*x*x", .72, .84);
