// for figuring out three equal bins of nu. you change the parameters of pT2 for each target (LC, SC, LPb, SPb), until the .getIntegral() for each bin/histogram is about the same
...
// change values of nu here until each histogram (hz1, hz2, hz3) all have about the same amount of data in them (.getIntegral() is about the same)
...
/*
 	 double piPlusPx2 = piPlusV.px() * piPlusV.px();
	 double piMinusPx2 = piMinusV.px() * piMinusV.px();
 	 double piZeroPx2 = gamma0V.px() * gamma0V.px();


	 double px2 = piPlusPx2 + piMinusPx2 + piZeroPx2;


	 double piPlusPy2 = piPlusV.py() * piPlusV.py();
	 double piMinusPy2 = piMinusV.py() * piMinusV.py();
 	 double piZeroPy2 = gamma0V.py() * gamma0V.py();


	 double py2 = piPlusPy2 + piMinusPy2 + piZeroPy2;

	 double pT2 = px2 + py2;
*/


	 double piPlusPx = piPlusV.px();
	 double piMinusPx = piMinusV.px();
 	 double piZeroPx = gamma0V.px();

 	 double totalPx = piPlusPx + piMinusPx + piZeroPx;

	 double totalPx2 = totalPx * totalPx;


 	 double piPlusPy = piPlusV.py();
	 double piMinusPy = piMinusV.py();
 	 double piZeroPy = gamma0V.py();

 	 double totalPy = piPlusPy + piMinusPy + piZeroPy;

	 double totalPy2 = totalPy * totalPy;

	 double pT2 = totalPx2 + totalPy2;



// change values of pT2 here until each histogram (hz1, hz2, hz3) all have about the same amount of data in them (.getIntegral() is about the same)


 		 if(W>2.0&&Q2>1.0&&y<0.85&&pT2<0.053){
			filter3++;
 		...
 		if(W>2.0&&Q2>1.0&&y<0.85&&pT2>=.053&&pT2<.151){
			filter3++;
			vL_omega.copy(vL_pi0_1);
			vL_omega.add(piPlus.vector());
			vL_omega.add(piMinus.vector());
		...
		if(W>2.0&&Q2>1.0&&y<0.85&&pT2>=.151){
			filter3++;
			vL_omega.copy(vL_pi0_1);
			vL_omega.add(piPlus.vector());
			vL_omega.add(piMinus.vector());
...
// creates histo of invariant mass for lowest bin of nu
...
// creates histo of invariant mass for middle bin of nu
...
// creates histo of invariant mass for highest bin of nu
