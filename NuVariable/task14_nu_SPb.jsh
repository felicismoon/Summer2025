...
// different ways of getting these limits
	...
	// double theZh = totalE / nu;
 		...
 		if(W>2.0&&Q2>1.0&&y<0.85&&nu>=6.6&&nu<7.9){
			filter3++;
			vL_omega.copy(vL_pi0_1);
			vL_omega.add(piPlus.vector());
			vL_omega.add(piMinus.vector());
...
F1D f4 = new F1D("f4","[amp]*gaus(x,[mean],[sigma])", .71, .84);
...
//F1D f10 = new F1D("f10","[amp]*gaus(x,[mean],[sigma])+[B0]+[B1]*x+[B2]*x*x", .68, .84);
...
//F1D f11 = new F1D("f11","[amp]*gaus(x,[mean],[sigma])", .68, .81);


F1D f11 = new F1D("f11","[amp]*gaus(x,[mean],[sigma])", .775, .82);
